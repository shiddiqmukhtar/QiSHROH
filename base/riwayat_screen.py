...
class Tab(MDFloatLayout, MDTabsBase):
    ...
    #carousel = AlfatihahScreen()

    
class RiwayatScreen(Screen):
   
    title = StringProperty ("Riwayat Imam")
    qory_name = QoryName()
    ...
    def on_kv_post(self, base_widget):
        ...
        for tab_name in self.qory_name.name_qory:
            ...
            #tab = Tab(text = f"[b][ref={tab_name}][b][font={fonts[-1]['fn_regular']}]{md_icons['circle-medium']}[/font][/ref][/b]  {tab_name}")
            tabs.add_widget(tab)

    def on_tab_switch(self,instance_tabs,instance_tab,instance_tab_label,tab_text):
            
        instance_tab.ids.label.text = self.qory_name.asysyatiby_text
